    ...
    namespace parser
    {
        class Ast2DotParser
        {
          public:
             ...
             * If ix is -1, add a new null else return indexed one
             */
            virtual std::string public_to_name(int const& ix = -1, int* = (int*)NULL);

            /*
             * Provide a label for a public vertex.
             */
            virtual std::string public_to_label(std::string const&);

	    /*
	     * Provide the index of a public node registered
	     */
	    virtual std::string public_to_index(std::string const&);
	    ...
	    /*
          ...
          private:
            ...
            // Public vertex are also numbered (in addr)
            std::map<int, std::string> _name2public;
