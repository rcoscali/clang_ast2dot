...
bool Ast2DotMain::create_graph(std::string& parent_name,
		std::string& previous_scstr, int level) {
	bool goes_on = true;
	std::string inbuf;
	...
	while (!std::cin.eof() && goes_on) {
		...
		if (!inbuf.empty()) {
			...
			// If not root (line doesn't start with a className), add quotes for child/sibling preprend string
...
int Ast2DotMain::do_main(int opt_ind) {
	...
	do {
		...
		std::string previous_scstr;
		int level = 0;
		(void) create_graph(parent_name, previous_scstr, level);
